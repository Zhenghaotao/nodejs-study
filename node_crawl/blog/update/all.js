...
async.series([
         ...
         */
            ...
            async.eachSeries(classList, function (c, next) {
                read.articleList(c.url, function (err, list) {
                    ...
                    //debug('看看每个id    list: %s     %s' , c.id, list.length);
                    next(err);
                });
            }, done);
        },
         ...
         */
            ...
            async.eachSeries(Object.keys(articleList), function (classId, next) {
                save.articleList(classId, articleList[classId], next);//list没有数据
            }, done);
        },
         ...
         */
            function (done) {
            async.eachSeries(Object.keys(articleList), function (classId, next) {
                //debug('看看每个classId    list: %s   %s' , classId ,articleList[classId]);
